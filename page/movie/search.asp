...
sub king_def()
	...
	searchfield="ktitle,kdescription,actor,director"
	...
	if len(query)>0 then
		...
		sql="select top 1000 kid,listid,ktitle,kdescription,kdate,kgrade,kpath from king__movie_page where "&query&" order by kid desc;"
		qcount=conn.execute("select count(kid) from king__movie_page where kshow=1 and "&query&";")(0)
		...
		set dp=new record
			...
			if dp.length>=0 then
				...
				for i=0 to dp.length
					set rs=conn.execute("select listname,listpath from king__movie_list where listid="&dp.data(1,i)&";")
	...
	king.outhtm king_default_template,"",king.invalue
