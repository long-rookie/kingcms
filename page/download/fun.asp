...
private sub class_initialize()
	...
	if king.checkcolumn("king__download_page")=false then
		install:update
	else
		set rs=conn.execute("select kversion from king__download_config where systemname='KingCMS'")
...
'	out listid
	...
	Il "]"
		...
		if len(listid)>0 and cstr(listid)<>"0" then
			...
			if conn.execute("select count(listid) from king__download_list where listid1="&listid&";")(0)>0 then
...
private function list_guide(l1)
	...
	set rs=conn.execute("select listid1,listname from king__download_list where listid="&l1&";")
		if not rs.eof and not rs.bof then
...
private function guide(l1)
	...
	set rs=conn.execute("select listid1,listname,listpath from king__download_list where listid="&l1&";")
		if not rs.eof and not rs.bof then
...
public sub createpage(l1)
	...
	set rs=conn.execute("select "&sql&" from king__download_page where kid in ("&l1&") and kpath<>'';")
		if not rs.eof and not rs.bof then
			data=rs.getrows()
		else
			redim data(0,-1)
	...
	for i=0 to ubound(data,2)
		if cstr(data(1,i))<>cstr(listid) then
			set rs=conn.execute("select listname,listpath,pagetemplate1,pagetemplate2,listcontent from king__download_list where listid="&data(1,i)&";")
				if not rs.eof and not rs.bof then
					datalist=rs.getrows()
					listid=data(1,i)
...
public sub createlist(l1)
	...
	set rs=conn.execute("select "&sql&" from king__download_list where listid in ("&l1&");")
		if not rs.eof and not rs.bof then
			datalist=rs.getrows()
		else
			redim datalist(0,-1)
...
'	sql="kid,listid,ktitle,kfrom,kdescription,kdate,kkeywords,kauthor,kpath,kimg,kgrade"'10 data
	...
	for j=0 to ubound(datalist,2)
		...
		set rs=conn.execute("select "&sql&" from king__download_page  where kshow=1 and listid="&datalist(0,j)&" or listids like '%,"&datalist(0,j)&",%' order by kup desc,korder "&jsorder&",kid "&jsorder&";")
			if not rs.eof and not rs.bof then
				data=rs.getrows()
...
'	sql="kid,listid,ktitle,kfrom,kdescription,kdate,kkeywords,kauthor,kpath,kimg,kgrade"'10 data
	...
	set dp=new record
		dp.pid=l2
		dp.rn=jsnumber
		dp.create "select "&sql&" from king__download_page where kshow=1 and listid="&datalist(0,0)&" or listids like '%,"&datalist(0,0)&",%' order by kup desc,korder "&jsorder&",kid "&jsorder&";"
		...
		if dp.length>-1 then
			...
			for i=0 to length'开始循环列表
				...
				king.value "path",encode(getpath(dp.data(0,i),dp.data(7,i),king.inst&datalist(2,0)&"/"&dp.data(5,i)))
				...
				if len(sql_field)>0 then
					...
					for k=0 to ubound(fields)
						if king.instre(sql_fieldhtml,fields(k)) then
							king.value kctag(fields(k)),encode(dp.data(k+13,i))
						else
							king.value kctag(fields(k)),encode(htmlencode(dp.data(k+13,i)))
						end if
					next
				end if
...
public sub createmap()
	...
	set rs=conn.execute("select top "&king.mapnumber&" kdate,kpath,kid,listid,kcommend,khead from king__download_page where kshow=1 and kgrade=0 order by kid desc;")
		if not rs.eof and not rs.bof then
			data=rs.getrows()
			...
			for i=0 to ubound(data,2)
				if cstr(listid)<>cstr(data(3,i)) then
					listid=data(3,i)
					set irs=conn.execute("select listpath from king__download_list where listid="&listid&";")
...
private function lastpage(l1,l2,l3,l4,l5,l6)'上一页
	if instr(lcase(l5),"{king:lastpage")>0 then
		dim rs,I1
		set rs=conn.execute("select top 1 ktitle,kgrade,kpath,kid from king__download_page where kshow=1 and listid="&l3&" and korder<"&l2&" order by korder desc,kid desc;")
			if not rs.eof and not rs.bof then
...
private function nextpage(l1,l2,l3,l4,l5)'kid,korder,listid,listpath,tmphtm //下一页
	if instr(lcase(l5),"{king:nextpage")>0 then
		dim rs,I1
		set rs=conn.execute("select top 1 ktitle,kgrade,kpath,kid from king__download_page where kshow=1 and listid="&l3&" and korder>"&l2&" order by korder asc,kid asc;")
			if not rs.eof and not rs.bof then
				I1="<a href="""&getpath(rs(3),rs(1),king.inst&l4&"/"&rs(2))&""">"&htmlencode(rs(0))&"</a>"
'			out I1
			else'如果不存在，则输出js加载来验证
...
public function getgrade(l1,l2)'返回会员组名称  l1 groupnum l2
	...
	case else
		set rs=conn.execute("select groupname from king__download_group where groupnum="&l1&";")
			if not rs.eof and not rs.bof then
				I1=htmlencode(rs(0))
			else
				I1=kc.lang("list/grade1")
				conn.execute "update king__download_page set kgrade=1 where kid="&l2&";"
			end if
			rs.close
		set rs=nothing
	end select
...
public sub head(l1)'会员访问权限验证 1:groupnum
	...
	case else
		king.pphead 1
		set rs=conn.execute("select groupuser from king__download_group where groupnum="&l1&";")
			if not rs.eof and not rs.bof then
				if king.instre(rs(0),king.name)=false then'如果不属于用户组，就提示错误
					king.error kc.lang("error/nogroup")
				end if
			else
				conn.execute "update king__download_page set kgrade=1 where kgrade="&l1&";"
			end if
			rs.close
		set rs=nothing
	end select
...
' king__download_page 
	...
	conn.execute "create table king__download_page ("&sql&")"
...
'king__download_list
	...
	conn.execute "create table king__download_list ("&sql&")"

' king__download_group
	...
	conn.execute "create table king__download_group ("&sql&")"
' king__download_config
	sql="systemname nvarchar(10),"
	sql=sql&"kversion real not null default 1"
	conn.execute "create table king__download_config ("&sql&")"

	conn.execute "insert into king__download_config (systemname) values ('KingCMS');"
	king.createmap
end sub
'  *** Copyright &copy KingCMS.com All Rights Reserved ***
public sub update()
	...
	conn.execute "alter table king__download_page add kc_downloadversion nvarchar(255) "
	conn.execute "alter table king__download_page add kc_demopath nvarchar(255) "
	conn.execute "alter table king__download_page add kc_image nvarchar(255) "
	conn.execute "alter table king__download_page add kc_downloadpath nvarchar(255) "
	conn.execute "alter table king__download_page add kc_size nvarchar(255) "
	conn.execute "alter table king__download_page add kc_content ntext "
	conn.execute "alter table king__download_list add listcontent ntext "

	conn.execute "update king__download_config set kversion="&r_thisver&" where systemname='KingCMS';"
end sub
...
'  *** Copyright &copy KingCMS.com All Rights Reserved ***
function king_tag_download_list(tag,invalue)
	on error resume next
	...
	case"current"
		if validate(listid,2) then
			set rs=conn.execute("select listid1 from king__download_list where listid="&listid&" order by listorder desc,listid desc;")
				if not rs.eof and not rs.bof then
	...
	set rs=conn.execute("select listid,listname,listpath,listdescription,listcontent from king__download_list where "&insql_id1&" order by listorder desc,listid desc;")
		if not rs.eof and not rs.bof then
			data=rs.getrows()
			for i=0 to ubound(data,2)
	...
	king_tag_download_list=tmplist

end function
'  *** Copyright &copy KingCMS.com All Rights Reserved ***
function king_tag_download_double(tag,invalue)
	on error resume next
	'需要获得
	dim jslistid,jshtm
	dim rs,data,i,sql
	...
	if validate(jslistid,6) or king.instre("sub",jslistid) then
		...
		case"sub"
			listid=king.getvalue(invalue,"listid")
			insql_id=king_tag_download_getsublist(listid)
		case else
		...
		if len(jslistname)>0 then
			set rs=conn.execute("select listid from king__download_list where "&king.likey("listname",jslistname)&" order by listorder desc,listid desc;")
				if not rs.eof and not rs.bof then
					data=rs.getrows()
					for i=0 to ubound(data,2)
	...
	set rs=conn.execute("select "&sql&" from king__download_list where "&insql_id&" order by listorder desc,listid desc;")
		if not rs.eof and not rs.bof then
			data=rs.getrows()
		else
			redim data(0,-1)
	...
	king_tag_download_double=tmplist
end function
'  *** Copyright &copy KingCMS.com All Rights Reserved ***
function king_tag_download(tag,invalue)
	on error resume next'这个必须要开启

	if left(tag,2)="{{" then
		king_tag_download=king_tag_download_double(tag,invalue)
		exit function
	end if
...
'	sql=" kid,listid,ktitle,kfrom,kdescription,kdate,kkeywords,kauthor,kpath,kimg,kgrade"'10 data
	...
	if validate(jslistid,6) or king.instre("sub,current",jslistid) then
		...
		case"sub"'只调用当前栏目下面的，并不是所有的，若加入所有，需要递归，麻烦
			listid=king.getvalue(invalue,"listid")
			insql_id=" and "&king_tag_download_getsublist(listid)
		case"current" 
			listid=king.getvalue(invalue,"listid")
			if validate(listid,2) then
		...
		jslistname=king.getlabel(tag,"listname")
		if len(jslistname)>0 then
			set rs=conn.execute("select listid from king__download_list where "&king.likey("listname",jslistname)&";")
				if not rs.eof and not rs.bof then
					data=rs.getrows()
					for i=0 to ubound(data,2)
						if len(insql_id)>0 then
					...
					king_tag_download="<!-- ### KingCMS download Tag Error ### -- "&vbcrlf&tag&vbcrlf&" -->"
					exit function
				end if
				rs.close
			set rs=nothing
	...
	case"related"'相关文章
		...
		if len(tlikey)>0 then
			if validate(kid,2) then
				insql="select top "&tnumber&sql&" from king__download_page where kshow=1 "&tlikey&insql_id&insql_key&" and kid<>"&kid&" order by korder desc,kid desc;"
			else
				insql="select top "&tnumber&sql&" from king__download_page where kshow=1 "&tlikey&insql_id&insql_key&" order by korder desc,kid desc;"
			end if
		else
			exit function
		end if
	case"hot"'热门
		insql="select top "&tnumber&sql&" from king__download_page where kshow=1 "&insql_id&insql_key&insql_img&" order by khit desc,kid desc;"
	case"chill","cold"'冷门
		insql="select top "&tnumber&sql&" from king__download_page where kshow=1 "&insql_id&insql_key&insql_img&" order by khit asc,kid asc;"
	case"head"'头条
		insql="select top "&tnumber&sql&" from king__download_page where kshow=1 "&insql_id&insql_key&insql_img&" and khead=1 order by korder desc,kid desc;"
	case"commend"'推荐
		insql="select top "&tnumber&sql&" from king__download_page where kshow=1 "&insql_id&insql_key&insql_img&" and kcommend=1 order by korder desc,kid desc;"
	case"rnds"'随机
		insql="select top "&tnumber&sql&" from king__download_page where kshow=1 "&insql_id&insql_key&insql_img&" order by rnd(-(kid+"&rnd()&"));"
	case"sql"'自定义的
		insql="select top "&tnumber&sql&" from king__download_page "&jssql
	case else '最新文章
		insql="select top "&tnumber&sql&" from king__download_page where kshow=1 "&insql_id&insql_key&insql_img&" order by korder desc,kid desc;"
	end select
...
'		sql="kid,listid,ktitle,kkeywords,kdescription,kpath,kdate,kgrade,korder,khit,kup,kcommend,khead"'12
	...
	king_tag_download=tmplist
end function
'  *** Copyright &copy KingCMS.com All Rights Reserved ***
function king_tag_download_getsublist(l1)
	on error resume next
	dim rs,data,i,I1
	if validate(l1,2) then
		set rs=conn.execute("select listid from king__download_list where listid1="&l1&";")
			if not rs.eof and not rs.bof then
				data=rs.getrows()
				for i=0 to ubound(data,2)
					if len(I1)>0 then
	...
	king_tag_download_getsublist=I1
end function
