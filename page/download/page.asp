...
sub king_def()
	...
	set rs=conn.execute("select listid,kgrade,kpath from king__download_page where kshow=1 and kid="&kid&";")
		if not rs.eof and not rs.bof then

			kc.head rs(1)

			set irs=conn.execute("select listpath from king__download_list where listid="&rs(0)&";")
...
sub king_nextpage()
	...
	set rs=conn.execute("select listid,korder from king__download_page where kid="&kid&";")
		if not rs.eof and not rs.bof then
			listid=rs(0)
			korder=rs(1)
		else
	...
	set rs=conn.execute("select listpath,listname from king__download_list where listid="&listid&";")
		if not rs.eof and not rs.bof then
			listpath=rs(0)
			listname=rs(1)
		else
	...
	set rs=conn.execute("select top 1 ktitle,kgrade,kpath from king__download_page where kshow=1 and listid="&listid&" and korder>"&korder&" order by korder asc,kid asc;")
		if not rs.eof and not rs.bof then
			kc.createpage kid
...
sub king_hit()
	...
	set rs=conn.execute("select khit from king__download_page where kid="&kid&";")
		if not rs.eof and not rs.bof then

			cookie=request.cookies("download")("count")
			if king.instre(cookie,kid)=false then'如果列表中没有这个id就++
				conn.execute "update king__download_page set khit=khit+1 where kid="&kid&";"
				if len(cookie)>0 then
					response.cookies("download")("count")=cookie&","&kid
				else
					response.cookies("download")("count")=kid
				end if
			end if
