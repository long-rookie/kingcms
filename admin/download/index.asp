...
sub king_search()
	...
	sql="select "&sql&" from king__download_page where "&query&" order by kdate desc;"
...
'		out sql
		...
		for i=0 to dp.length
			if cstr(listid)<>cstr(dp.data(9,i)) then
				listid=dp.data(9,i)
				set rs=conn.execute("select listpath from king__download_list where listid="&listid&";")
...
sub king_def()
	...
	sql="select listid,listname,listpath from king__download_list where listid1="&listid&" order by listorder desc,listid desc;"
	...
	set dp=new record
		...
		for i=0 to dp.length
			sublist=conn.execute("select count(listid) from king__download_list where listid1="&dp.data(0,i)&";")(0)
			if sublist=0 then
			...
			listcount=conn.execute("select count(kid) from king__download_page where listid="&dp.data(0,i)&";")(0)
...
function king_tree(l1,l2)
	...
	set rs=conn.execute("select listid,listname from king__download_list where listid1="&l1&" order by listorder asc,listid")
		if not rs.eof and not rs.bof then
			data=rs.getrows()
			for i=0 to ubound(data,2)
				I1=I1&"ll("&data(0,i)&",'"&htm2js(data(1,i))&"',"&l2&");"
				if conn.execute("select count(listid) from king__download_list where listid1="&data(0,i)&";")(0)>0 then
...
sub king_group()
	...
	set rs=conn.execute("select groupid,groupnum,groupname from king__download_group order by groupnum;")
		if not rs.eof and not rs.bof then
			data=rs.getrows()
			for i=0 to ubound(data,2)
...
sub king_setgroup()
	...
	if len(groupid)>0 then
		...
		conn.execute "delete from king__download_group where groupid="&groupid&";"
...
sub king_edtgroup()
	...
	if king.ismethod or len(groupid)=0 then
		...
		if king.ismethod=false then
			if conn.execute("select count(groupid) from king__download_group;")(0)>0 then
				data(0,0)=conn.execute("select max(groupnum) from king__download_group;")(0)+1
			else
				data(0,0)=2
			end if
		end if
	else
		set rs=conn.execute("select "&sql&" from king__download_group where groupid="&groupid&";")
			if not rs.eof and not rs.bof then
				data=rs.getrows()
			else
				king.error king.lang("error/invalid")
	...
	if len(groupid)>0 then
		checknum="groupnum|6|"&encode(kc.lang("check/num"))&"|1-5;groupnum|9|"&encode(kc.lang("check/num1"))&"|select count(groupid) from king__download_group where groupnum=$pro$ and groupid<>"&groupid&";groupnum|2|"&encode(kc.lang("check/num2"))&";"&isnum&"|13|"&encode(kc.lang("check/num3"))
	else
		checknum="groupnum|6|"&encode(kc.lang("check/num"))&"|1-5;groupnum|9|"&encode(kc.lang("check/num1"))&"|select count(groupid) from king__download_group where groupnum=$pro$;groupnum|2|"&encode(kc.lang("check/num2"))&";"&isnum&"|13|"&encode(kc.lang("check/num3"))
	end if
	...
	if king.ischeck and king.ismethod then
		if len(groupid)>0 then
			conn.execute "update king__download_group set groupnum="&data(0,0)&",groupname='"&safe(data(1,0))&"',groupuser='"&safe(data(2,0))&"' where groupid="&groupid
		else
			conn.execute "insert into king__download_group ("&sql&") values ("&data(0,0)&",'"&safe(data(1,0))&"','"&safe(data(2,0))&"')"
		end if
...
sub king_field()
	...
	listid=quest("listid",2)
	if len(listid)=0 then king.error download("error/listid")
	...
	sql="select "&sql&" from king__download_page where listid="&listid&" order by korder desc,kid desc;"
...
sub king_create()
	...
	kid=quest("kid",2)
	set rs=conn.execute("select kid,listid,kpath,kgrade from king__download_page where kid="&kid)
		if not rs.eof and not rs.bof then
			kc.createpage kid

			listpath=conn.execute("select listpath from king__download_list where listid="&rs(1))(0)
			if cstr(rs(3))="0" then
	...
	select case quest("submits",0)
		...
		case"createlist","createlists"
			...
			if lcase(quest("submits",0))=lcase("createlists") then
				set rs=conn.execute("select listid from king__download_list order by listid;")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
		...
		case "createpage"
			...
			set dp=new record
				...
				dp.create "select kid from king__download_page where listid in ("&listid&") and kshow=1 order by kid,listid;"
				if dp.length>-1 then
					for i=0 to dp.length
						j=Timer
		...
		case"createpages"
			II "<script>window.parent.progress_show();</script>"
			set dp=new record
				dp.create "select kid from king__download_page where kshow=1 order by kid,listid;"
				for i=0 to dp.length
						j=Timer
						kc.createpage dp.data(0,i)
						pct=int((fix(dp.rn*(dp.pid-1)+i+1)/dp.count)*100)
		...
		case"createnotfile"
			...
			if len(listid)>0 then
				...
				if len(kid)=0 then
					set dp=new record
						dp.create "select kid,kpath,listid from king__download_page where listid in ("&listid&");"
						for i=0 to dp.length
							if cstr(dp.data(2,i))<>cstr(listid) then
								listpath=conn.execute("select listpath from king__download_list where listid="&dp.data(2,i)&";")(0)
							end if
				...
				if len(kid)>0 then
					set dp=new record
						dp.create "select kid from king__download_page where kid in ("&kid&") and kshow=1 order by kid,listid;"
						for i=0 to dp.length
							j=Timer
							kc.createpage dp.data(0,i)
							pct=int((fix(dp.rn*(dp.pid-1)+i+1)/dp.count)*100)
...
function getlistpidcount(l1)
	...
	set rs=conn.execute("select "&sql&" from king__download_list where listid ="&l1&";")
		if not rs.eof and not rs.bof then
			datalist=rs.getrows()
		else
			redim datalist(0,-1)
	...
	set rs=conn.execute("select kid from king__download_page where kshow=1 and listid="&datalist(0,0)&" or listids like '%,"&datalist(0,0)&",%' order by kup desc,korder "&jsorder&",kid "&jsorder&";")
		if not rs.eof and not rs.bof then
			data=rs.getrows()
			'初始化变量值
...
sub king_set()
	...
	case"show","up","commend","head"
		...
		conn.execute "update king__download_page set k"&form("submits")&"="&tag&" where kid="&id&";"
	...
	case"show0","show1","up0","up1","commend1","commend0","head0","head1"
		if len(list)>0 then
			...
			conn.execute "update king__download_page set k"&ksubmit&"="&kis&" where kid in ("&list&");"
	...
	case"union"
		if len(list)>0 then
			...
			if len(newlist)=0 then
				...
				set rs=conn.execute("select listid,listname from king__download_list where listid in ("&list&") order by listorder desc")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
				...
				'删除原数据
				set rs=conn.execute("select listid,listpath from king__download_list where listid in ("&list&") and listid<>"&newlist&";")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
					...
					rs.close
				set rs=nothing
				conn.execute "update king__download_page set listid="&newlist&" where listid in ("&list&");"
				kc.createlist list
	...
	case"moveto"
		if len(list)>0 then
			...
			if len(newlist)=0 then
				...
				'读取删除目录
				set rs=conn.execute("select listid,kid,kpath from king__download_page where kid in ("&list&")")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
							if listid<>data(0,i) then
								listpath=conn.execute("select listpath from king__download_list where listid="&data(0,i)&";")(0)
								listid=data(0,i)
							end if
				...
				set rs=nothing
				'数据移动
				conn.execute "update king__download_page set listid="&newlist&" where kid in ("&list&");"
				'重新生成文章
				kc.createpage list
				'生成列表
	...
	case"delete"
		if len(list)>0 then
			if action="set" then
				set rs=conn.execute("select kid,listid,kpath from king__download_page where kid in ("&list&");")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
							if cstr(listid)<>cstr(data(1,i)) then
								listpath=conn.execute("select listpath from king__download_list where listid="&data(1,i)&";")(0)
								listid=data(1,i)
							end if
							if king.isfile(data(2,i)) then'文件
								for j=1 to 50
						...
						next
						conn.execute "delete from king__download_page where kid in ("&list&");"
				...
				set rs=conn.execute("select listid,listpath from king__download_list where listid in ("&list&");")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
							if conn.execute("select count(*) from king__download_list where listid1="&data(0,i)&";")(0)>0 then
								king.flo kc.lang("flo/notdel"),1
							end if
							king.deletefolder "../../"&data(1,i)
							conn.execute "delete from king__download_list where listid="&data(0,i)&";"
							conn.execute "delete from king__download_page where listid="&data(0,i)&";"
						next
					else
						king.flo kc.lang("flo/invalid"),1
					end if
...
sub king_edt()
	...
	sql_field="kc_author,kc_downloadversion,kc_demopath,kc_image,kc_downloadpath,kc_size,kc_content"
	...
	if king.ismethod or len(kid)=0 then
		...
		set rs=conn.execute("select "&sql&" from king__download_page where kid="&kid&";")
			if not rs.eof and not rs.bof then
				data=rs.getrows()
			else
				king.error king.lang("error/invalid")
	...
	if conn.execute("select count(listid) from king__download_list;")(0)=0 then
	...
	Il " - "
	if cstr(form("snapimg"))="1" then checked=" checked=""checked""" else checked=""
	...
	if cstr(form("checktitle"))="1" then
		if len(kid)>0 then
			checktitle="ktitle|6|"&encode(kc.lang("check/title"))&"|1-100;ktitle|9|"&encode(kc.lang("check/title1"))&"|select count(kid) from king__download_page where listid="&listid&" and ktitle='$pro$' and kid<>"&kid
		else
			checktitle="ktitle|6|"&encode(kc.lang("check/title"))&"|1-100;ktitle|9|"&encode(kc.lang("check/title1"))&"|select count(kid) from king__download_page where listid="&listid&" and ktitle='$pro$'"
		end if
	else
	...
	'author
	Il "<p><label>"&kc.lang("label/author")&"<i>{king:author/}</i></label><input maxlength="""" type=""text"" name=""kc_author"" id=""kc_author"" value="""&formencode(data(10,0))&""" class=""in3"" />"
	Il king.form_eval("kc_author","不详")
	Il king.form_eval("kc_author","Sin.CS")
	Il king.check("kc_author|0|"&encode(kc.lang("check/author1")))
	Il "</p>"
	'downloadversion
	Il "<p><label>"&kc.lang("label/downloadversion")&"<i>{king:downloadversion/}</i></label><input maxlength="""" type=""text"" name=""kc_downloadversion"" id=""kc_downloadversion"" value="""&formencode(data(11,0))&""" class=""in2"" />"
	Il king.form_eval("kc_downloadversion","1.0")
	Il king.form_eval("kc_downloadversion","2.0")
	Il king.form_eval("kc_downloadversion","3.0")
	Il king.form_eval("kc_downloadversion","5.0")
	Il king.check("kc_downloadversion|0|"&encode(kc.lang("check/downloadversion1")))
	Il "</p>"
	'demopath
	Il "<p><label>"&kc.lang("label/demopath")&"<i>{king:demopath/}</i></label><input maxlength="""" type=""text"" name=""kc_demopath"" id=""kc_demopath"" value="""&formencode(data(12,0))&""" class=""in4"" />"
if len(form("kc_demopath"))>0 then
	Il king.check("kc_demopath|5|"&encode(kc.lang("check/demopath1")))
end if
	Il "</p>"
	'image
	Il "<p><label>"&kc.lang("label/image")&"<i>{king:image/}</i></label><input maxlength="""" type=""text"" name=""kc_image"" id=""kc_image"" value="""&formencode(data(13,0))&""" class=""in4"" />"
...
if len(form("kc_image"))>0 then
	...
	Il "</p>"
	'downloadpath
	Il "<p><label>"&kc.lang("label/downloadpath")&"<i>{king:downloadpath/}</i></label><input maxlength="""" type=""text"" name=""kc_downloadpath"" id=""kc_downloadpath"" value="""&formencode(data(14,0))&""" class=""in4"" />"
	king.form_brow "kc_downloadpath","chm/doc/pdf/zip/rar/tar/gz/bzip2/gif/jpg/jpeg/png"
if len(form("kc_downloadpath"))>0 then
	Il king.check("")
end if
	Il "</p>"
	'size
	Il "<p><label>"&kc.lang("label/size")&"<i>{king:size/}</i></label><input maxlength="""" type=""text"" name=""kc_size"" id=""kc_size"" value="""&formencode(data(15,0))&""" class=""in1"" />"
	Il king.form_eval("kc_size","不详")
if len(form("kc_size"))>0 then
	Il king.check("")
end if
	Il "</p>"
	'content
	Il king.form_editor("kc_content",kc.lang("label/content")&"<i>{king:content/}</i>",data(16,0),"kc_content|0|"&encode(kc.lang("check/content1")))

	'kpath
	if k_is(2)=1 then
		if len(kid)>0 then'更新
			checkpath=";kpath|9|"&encode(kc.lang("check/path2"))&"|select count(kid) from king__download_page where listid="&listid&" and kpath='$pro$' and kid<>"&kid
		else
			checkpath=";kpath|9|"&encode(kc.lang("check/path2"))&"|select count(kid) from king__download_page where listid="&listid&" and kpath='$pro$'"
		end if
		...
		if len(kid)=0 then
			maxkid=king.neworder("king__download_page","kid")
		...
		if king.instre(king.plugin,"passport") then
			...
			set rs=conn.execute("select groupnum,groupname from king__download_group order by groupnum")
				if not rs.eof and not rs.bof then
					idata=rs.getrows()
	...
	if king.ischeck and king.ismethod then
		...
		if cstr(form("snapimg"))="1" or cstr(form("snapimg"))="" then data(16,0)=king.snap(data(16,0))
		...
		if data(8,0)="MD5" then
			if len(kid)>0 then
				...
				data(8,0)=md5(salt(10)&king.neworder("king__download_page","korder"),0)
			end if
		end if
		'listids
...
'	sql="ktitle,kshow,kcommend,kup,khead,kgrade,kkeywords,kdescription,kpath,listids,kc_author,kc_downloadversion,kc_demopath,kc_image,kc_downloadpath,kc_size,kc_content"
		...
		if len(kid)>0 then
			...
			conn.execute "update king__download_page set ktitle='"&safe(data(0,0))&"',kshow="&safe(data(1,0))&",kcommend="&safe(data(2,0))&" ,kup="&safe(data(3,0))&",khead="&safe(data(4,0))&",kgrade="&safe(data(5,0))&",kpath='"&safe(data(8,0))&"',listids='"&safe(data(9,0))&"',listid="&listid&",kdescription='"&safe(kdescription)&"',kkeywords='"&safe(kkeywords)&"'"&insql&" where kid="&kid&";"
		else
			if len(sql_field)>0 then
				fields=split(sql_field,",")
				for i=0 to ubound(fields)
			...
			conn.execute "insert into king__download_page ("&sql&",kdate,korder,listid) values ('"&safe(data(0,0))&"',"&safe(data(1,0))&","&safe(data(2,0))&","&safe(data(3,0))&","&safe(data(4,0))&","&safe(data(5,0))&",'"&safe(kkeywords)&"','"&safe(kdescription)&"','"&safe(data(8,0))&"','"&safe(data(9,0))&"'"&insql&",'"&tnow&"',"&king.neworder("king__download_page","korder")&","&listid&")"
			kid=king.newid("king__download_page","kid")

			set rs=conn.execute("select listname,listpath from king__download_list where listid="&listid&";")
				if not rs.eof and not rs.bof then
					datalist=rs.getrows()
				end if
				rs.close
...
'			king.createrss
		...
		'neworder：king.neworder("king__download_page","korder")
		kc.createpage kid
...
sub king_edtlist()
	...
	if king.ismethod or len(listid)=0 then
		...
		set rs=conn.execute("select "&sql&" from king__download_list where listid="&listid&";")
			if not rs.eof and not rs.bof then
				data=rs.getrows()
			else
				king.error king.lang("error/invalid")
	...
	if len(listid)>0 then'更新
		checkpath="listpath|6|"&encode(kc.lang("check/path"))&"|1-100;listpath|15|"&encode(kc.lang("check/path1"))&";listpath|9|"&encode(kc.lang("check/path2"))&"|select count(listid) from king__download_list where listpath='$pro$' and listid<>"&listid
	else
		checkpath="listpath|6|"&encode(kc.lang("check/path"))&"|1-100;listpath|15|"&encode(kc.lang("check/path1"))&";listpath|9|"&encode(kc.lang("check/path2"))&"|select count(listid) from king__download_list where listpath='$pro$'"
	end if
	...
	if king.ischeck and king.ismethod then
		if len(listid)>0 then
			conn.execute "update king__download_list set listname='"&safe(data(0,0))&"',listpath='"&safe(data(1,0))&"',listtemplate1='"&safe(data(2,0))&"',listtemplate2='"&safe(data(3,0))&"',pagetemplate1='"&safe(data(4,0))&"',pagetemplate2='"&safe(data(5,0))&"',listid1="&safe(data(6,0))&",listkeyword='"&safe(data(7,0))&"',listdescription='"&safe(data(8,0))&"',listtitle='"&safe(data(9,0))&"',listcontent='"&safe(data(10,0))&"' where listid="&listid&";"
		else
			conn.execute "insert into king__download_list ("&sql&",listorder) values ('"&safe(data(0,0))&"','"&safe(data(1,0))&"','"&safe(data(2,0))&"','"&safe(data(3,0))&"','"&safe(data(4,0))&"','"&safe(data(5,0))&"',"&safe(data(6,0))&",'"&safe(data(7,0))&"','"&safe(data(8,0))&"','"&safe(data(9,0))&"','"&safe(data(10,0))&"',"&king.neworder("king__download_list","listorder")&")"
		end if
		kc.createmap
...
sub king_updown()
	...
	if len(kid)>0 then
		king.updown "king__download_page,kid,korder",kid,"listid="&listid
	else
		king.updown "king__download_list,listid,listorder",listid,0
	end if
end sub
