...
sub king_search()
	...
	sql="select "&sql&" from king__movie_page where "&query&" order by kdate desc;"
...
'		out sql
		...
		for i=0 to dp.length
			if cstr(listid)<>cstr(dp.data(9,i)) then
				listid=dp.data(9,i)
				set rs=conn.execute("select listpath from king__movie_list where listid="&listid&";")
...
sub king_def()
	...
	sql="select listid,listname,listpath from king__movie_list where listid1="&listid&" order by listorder desc,listid desc;"
	...
	set dp=new record
		...
		for i=0 to dp.length
			sublist=conn.execute("select count(listid) from king__movie_list where listid1="&dp.data(0,i)&";")(0)
			if sublist=0 then
			...
			listcount=conn.execute("select count(kid) from king__movie_page where listid="&dp.data(0,i)&";")(0)
...
function king_tree(l1,l2)
	...
	set rs=conn.execute("select listid,listname from king__movie_list where listid1="&l1&" order by listorder asc,listid")
		if not rs.eof and not rs.bof then
			data=rs.getrows()
			for i=0 to ubound(data,2)
				I1=I1&"ll("&data(0,i)&",'"&htm2js(data(1,i))&"',"&l2&");"
				if conn.execute("select count(listid) from king__movie_list where listid1="&data(0,i)&";")(0)>0 then
...
sub king_group()
	...
	set rs=conn.execute("select groupid,groupnum,groupname from king__movie_group order by groupnum;")
		if not rs.eof and not rs.bof then
			data=rs.getrows()
			for i=0 to ubound(data,2)
...
sub king_setgroup()
	...
	if len(groupid)>0 then
		...
		conn.execute "delete from king__movie_group where groupid="&groupid&";"
...
sub king_edtgroup()
	...
	if king.ismethod or len(groupid)=0 then
		...
		if king.ismethod=false then
			if conn.execute("select count(groupid) from king__movie_group;")(0)>0 then
				data(0,0)=conn.execute("select max(groupnum) from king__movie_group;")(0)+1
			else
				data(0,0)=2
			end if
		end if
	else
		set rs=conn.execute("select "&sql&" from king__movie_group where groupid="&groupid&";")
			if not rs.eof and not rs.bof then
				data=rs.getrows()
			else
				king.error king.lang("error/invalid")
	...
	if len(groupid)>0 then
		checknum="groupnum|6|"&encode(kc.lang("check/num"))&"|1-5;groupnum|9|"&encode(kc.lang("check/num1"))&"|select count(groupid) from king__movie_group where groupnum=$pro$ and groupid<>"&groupid&";groupnum|2|"&encode(kc.lang("check/num2"))&";"&isnum&"|13|"&encode(kc.lang("check/num3"))
	else
		checknum="groupnum|6|"&encode(kc.lang("check/num"))&"|1-5;groupnum|9|"&encode(kc.lang("check/num1"))&"|select count(groupid) from king__movie_group where groupnum=$pro$;groupnum|2|"&encode(kc.lang("check/num2"))&";"&isnum&"|13|"&encode(kc.lang("check/num3"))
	end if
	...
	if king.ischeck and king.ismethod then
		if len(groupid)>0 then
			conn.execute "update king__movie_group set groupnum="&data(0,0)&",groupname='"&safe(data(1,0))&"',groupuser='"&safe(data(2,0))&"' where groupid="&groupid
		else
			conn.execute "insert into king__movie_group ("&sql&") values ("&data(0,0)&",'"&safe(data(1,0))&"','"&safe(data(2,0))&"')"
		end if
...
sub king_field()
	...
	listid=quest("listid",2)
	if len(listid)=0 then king.error movie("error/listid")
	...
	sql="select "&sql&" from king__movie_page where listid="&listid&" order by korder desc,kid desc;"
...
sub king_create()
	...
	kid=quest("kid",2)
	set rs=conn.execute("select kid,listid,kpath,kgrade from king__movie_page where kid="&kid)
		if not rs.eof and not rs.bof then
			kc.createpage kid

			listpath=conn.execute("select listpath from king__movie_list where listid="&rs(1))(0)
			if cstr(rs(3))="0" then
	...
	select case quest("submits",0)
		...
		case"createlist","createlists"
			...
			if lcase(quest("submits",0))=lcase("createlists") then
				set rs=conn.execute("select listid from king__movie_list order by listid;")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
		...
		case "createpage"
			...
			set dp=new record
				...
				dp.create "select kid from king__movie_page where listid in ("&listid&") and kshow=1 order by kid,listid;"
				if dp.length>-1 then
					for i=0 to dp.length
						j=Timer
		...
		case"createpages"
			II "<script>window.parent.progress_show();</script>"
			set dp=new record
				dp.create "select kid from king__movie_page where kshow=1 order by kid,listid;"
				for i=0 to dp.length
						j=Timer
						kc.createpage dp.data(0,i)
						pct=int((fix(dp.rn*(dp.pid-1)+i+1)/dp.count)*100)
		...
		case"createnotfile"
			...
			if len(listid)>0 then
				...
				if len(kid)=0 then
					set dp=new record
						dp.create "select kid,kpath,listid from king__movie_page where listid in ("&listid&");"
						for i=0 to dp.length
							if cstr(dp.data(2,i))<>cstr(listid) then
								listpath=conn.execute("select listpath from king__movie_list where listid="&dp.data(2,i)&";")(0)
							end if
				...
				if len(kid)>0 then
					set dp=new record
						dp.create "select kid from king__movie_page where kid in ("&kid&") and kshow=1 order by kid,listid;"
						for i=0 to dp.length
							j=Timer
							kc.createpage dp.data(0,i)
							pct=int((fix(dp.rn*(dp.pid-1)+i+1)/dp.count)*100)
...
function getlistpidcount(l1)
	...
	set rs=conn.execute("select kid from king__movie_page where kshow=1 and listid="&datalist(0,0)&" or listids like '%,"&datalist(0,0)&",%' order by kup desc,korder "&jsorder&",kid "&jsorder&";")
		if not rs.eof and not rs.bof then
			data=rs.getrows()
			'初始化变量值
...
sub king_set()
	...
	case"introduction"
		if kc.xlintroduction("http://u.xunlei.com/xml/all.xml") then
			king.flo kc.lang("flo/introduction"),1
		else
			king.flo kc.lang("flo/introductionerr"),1
		end if
	case"close"
		conn.execute "update king__movie_config set introduction=0 where systemname='KingCMS';"
		king.flo kc.lang("flo/close")&"<br/>update king__movie_config set introduction=1",1
	case"cpid"
		cpid=form("cpid")
		iscpid=validate(cpid,2)

		if iscpid then
			conn.execute "update king__movie_config set cpid="&cpid&" where systemname='KingCMS';"
			king.flo kc.lang("flo/upcpid"),1
		else
			king.ol="<div class=""k_form"">"
			king.ol="<p><label>"&kc.lang("common/cpid")&"</label><input type=""text"" value="""&kc.cpid&""" name=""cpid"" class=""in3"" />"
			if len(cpid)>0 then
				king.ol=king.check(iscpid&"|13|"&kc.lang("check/cpid"))
			end if
			king.ol="</p>"
			king.ol="<div class=""k_menu"">"
			king.ol="<input type=""button"" value="""&king.lang("common/up")&""" onclick=""javascript:posthtm('index.asp?action=set','flo','submits=cpid&cpid='+encodeURIComponent(document.getElementById('cpid').value));"" />"
			king.ol="<input type=""button"" value="""&king.lang("common/close")&""" onclick=""javascript:display('flo');""/>"
			king.ol="</div>"
			king.ol="</div>"
	...
	case"show","up","commend","head"
		...
		conn.execute "update king__movie_page set k"&form("submits")&"="&tag&" where kid="&id&";"
	...
	case"show0","show1","up0","up1","commend1","commend0","head0","head1"
		if len(list)>0 then
			...
			conn.execute "update king__movie_page set k"&ksubmit&"="&kis&" where kid in ("&list&");"
	...
	case"union"
		if len(list)>0 then
			...
			if len(newlist)=0 then
				...
				set rs=conn.execute("select listid,listname from king__movie_list where listid in ("&list&") order by listorder desc")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
				...
				'删除原数据
				set rs=conn.execute("select listid,listpath from king__movie_list where listid in ("&list&") and listid<>"&newlist&";")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
					...
					rs.close
				set rs=nothing
				conn.execute "update king__movie_page set listid="&newlist&" where listid in ("&list&");"
				kc.createlist list
				king.createhome
	...
	case"moveto"
		if len(list)>0 then
			...
			if len(newlist)=0 then
				...
				ol=ol&"<div class=""k_menu""><input type=""button"" value="""&kc.lang("list/move")&""" "
				ol=ol&"onclick=""javascript:posthtm('index.asp?action=set','flo','submits=moveto&listid='+encodeURIComponent(document.getElementById('listid').value)+'&list="&list&"');"" />"
				ol=ol&"<input type=""button"" value="""&king.lang("common/close")&""" onclick=""javascript:display('flo');""/>"
				ol=ol&"</div>"'end k_but
				...
				'读取删除目录
				set rs=conn.execute("select listid,kid,kpath from king__movie_page where kid in ("&list&")")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
							if listid<>data(0,i) then
								listpath=conn.execute("select listpath from king__movie_list where listid="&data(0,i)&";")(0)
								listid=data(0,i)
							end if
				...
				set rs=nothing
				'数据移动
				conn.execute "update king__movie_page set listid="&newlist&" where kid in ("&list&");"
				'重新生成文章
				kc.createpage list
				'生成列表
	...
	case"delete"
		if len(list)>0 then
			if action="set" then
				set rs=conn.execute("select kid,listid,kpath from king__movie_page where kid in ("&list&");")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
							if cstr(listid)<>cstr(data(1,i)) then
								listpath=conn.execute("select listpath from king__movie_list where listid="&data(1,i)&";")(0)
								listid=data(1,i)
							end if
							if king.isfile(data(2,i)) then'文件
								for j=1 to 50
						...
						next
						conn.execute "delete from king__movie_page where kid in ("&list&");"
				...
				set rs=conn.execute("select listid,listpath from king__movie_list where listid in ("&list&");")
					if not rs.eof and not rs.bof then
						data=rs.getrows()
						for i=0 to ubound(data,2)
							if conn.execute("select count(*) from king__movie_list where listid1="&data(0,i)&";")(0)>0 then
								king.flo kc.lang("flo/notdel"),1
							end if
							king.deletefolder "../../"&data(1,i)
							conn.execute "delete from king__movie_list where listid="&data(0,i)&";"
							conn.execute "delete from king__movie_page where listid="&data(0,i)&";"
						next
					else
						king.flo kc.lang("flo/invalid"),1
					end if
	...
	case"addu"'添加联盟库
		king.ol="<div class=""k_form"">"
		if validate(form("userver"),2) and len(form("upath"))>0 and cstr(form("is"))="1" then
			'对提交的数据进行格式化
			newupath=formatupath(form("userver"),form("upath"))

			king.ol="<p><label>"&kc.lang("common/userver")&"</label>"
			king.ol="<textarea name=""newupath"" id=""newupath"" rows=""21"" cols=""50"" class=""in5"">"&newupath&"</textarea></p>"
			king.ol="<input type=""hidden"" name=""upath"" value="""&htmlencode(form("upath"))&""" />"
			king.ol="<input type=""hidden"" name=""userver"" value="""&form("userver")&""" />"

			king.ol="<div class=""k_menu"">"
			king.ol="<input type=""button"" value="""&king.lang("common/insert")&""" onclick=""document.getElementById('kc_params').value=document.getElementById('newupath').value;display('aja');"" />"
			king.ol="<input type=""button"" value="""&king.lang("common/back")&""" onclick=""posthtm('index.asp?action=set','aja','submits=addu&is=0&userver='+encodeURIComponent(document.getElementById('userver').value)+'&upath='+encodeURIComponent(document.getElementById('upath').value));"" />"
			king.ol="<input type=""button"" value="""&king.lang("common/close")&""" onclick=""javascript:display('aja');"" />"
			king.ol="</div>"
		else
			king.ol="<p><label>"&kc.lang("common/userver")&"</label>"
			king.ol="<input type=""text"" name=""userver"" value="""&form("userver")&""" id=""userver"" name=""userver"" maxlength=""5"" class=""in2"" /></p>"
			king.ol="<p><label>"&kc.lang("common/upath")&"</label>"
			king.ol="<textarea name=""upath"" id=""upath"" rows=""19"" cols=""50"" class=""in5"">"&htmlencode(form("upath"))&"</textarea></p>"
			king.ol="<div class=""k_menu"">"
			king.ol="<input type=""button"" value="""&king.lang("common/submit")&""" onclick=""posthtm('index.asp?action=set','aja','submits=addu&is=1&userver='+encodeURIComponent(document.getElementById('userver').value)+'&upath='+encodeURIComponent(document.getElementById('upath').value));"" />"
			king.ol="<input type=""button"" value="""&king.lang("common/close")&""" onclick=""javascript:display('aja');"" />"
			king.ol="</div>"
		end if
		king.ol="</div>"

		king.aja kc.lang("common/addu"),king.writeol
	case"today"'每日更新
		kc.uptoday
		king.flo kc.lang("flo/today"),0
	end select
end sub
'  *** Copyright &copy KingCMS.com All Rights Reserved ***
function formatupath(l1,l2)'userver,upath
	dim I1(1),I2,I3,I4,i
	if len(trim(l2))>0 then
		I2=split(trim(l2),chr(13)&chr(10))
		'获得首项
		...
		for i=0 to ubound(I2)
			if len(I2(i))>0 then
				I4=I4&mid(I2(i),len(I1(0))+1,len(I2(i))-len(I1(0))-len(I1(1)))&"|"&l1&"|"&I2(i)&chr(13)&chr(10)
			end if
		next
		formatupath=I4
	end if
...
sub king_edt()
	...
	sql_field="kc_actor,kc_director,kc_image,kc_params,kc_content,kc_lastdate"
	...
	if king.ismethod or len(kid)=0 then
		...
		if king.ismethod=false then
			...
			data(15,0)=formatdate(now(),0)
		end if

	else
		set rs=conn.execute("select "&sql&" from king__movie_page where kid="&kid&";")
			if not rs.eof and not rs.bof then
				data=rs.getrows()
			else
				king.error king.lang("error/invalid")
	...
	kc.list
	Il "<link href=""http://play.kankan.xunlei.com/u/tma/css/style.css"" rel=""stylesheet"" type=""text/css"" />"
	Il "<script src=""http://play.kankan.xunlei.com/u/tma/js/tma.js"" type=""text/javascript"" charset=""gbk""></script>"
	Il "<script>"
	...
	if conn.execute("select count(listid) from king__movie_list;")(0)=0 then
	...
	Il "<input type=""checkbox"" value=""1"" name=""snapimg"" id=""snapimg"""&checked&"><label for=""snapimg"">"&kc.lang("label/snapimg")&"</label> "
	...
	if cstr(form("checktitle"))="1" then
		if len(kid)>0 then
			checktitle="ktitle|6|"&encode(kc.lang("check/title"))&"|1-100;ktitle|9|"&encode(kc.lang("check/title1"))&"|select count(kid) from king__movie_page where listid="&listid&" and ktitle='$pro$' and kid<>"&kid
		else
			checktitle="ktitle|6|"&encode(kc.lang("check/title"))&"|1-100;ktitle|9|"&encode(kc.lang("check/title1"))&"|select count(kid) from king__movie_page where listid="&listid&" and ktitle='$pro$'"
		end if
	else
		checktitle="ktitle|6|"&encode(kc.lang("check/title"))&"|1-100"
	end if
	king.form_input "ktitle",kc.lang("label/title")&" <i>{king:title/}</i>",data(0,0),checktitle

	'actor
	Il "<p><label>"&kc.lang("label/actor")&" <i>{king:actor/}</i></label><input maxlength="""" type=""text"" name=""kc_actor"" id=""kc_actor"" value="""&formencode(data(10,0))&""" class=""in4"" />"
	Il king.check("")
	Il "</p>"
	'director
	Il "<p><label>"&kc.lang("label/director")&" <i>{king:director/}</i></label><input maxlength="""" type=""text"" name=""kc_director"" id=""kc_director"" value="""&formencode(data(11,0))&""" class=""in4"" />"
	Il king.check("")
	Il "</p>"
	'image
	Il "<p><label>"&kc.lang("label/image")&" <i>{king:image/}</i></label><input maxlength="""" type=""text"" name=""kc_image"" id=""kc_image"" value="""&formencode(data(12,0))&""" class=""in4"" />"
	king.form_brow "kc_image","jpeg/jpg/png/gif"
	Il king.check("")
	Il "</p>"
	'params
	Il "<p><label>"&kc.lang("label/params")&" <a href=""javascript:;"" onclick=""posthtm('index.asp?action=set','aja','submits=addu')"">["&kc.lang("common/addu")&"]</a> <i>{king:params/}</i> <i>{king:param/}</i></label><textarea name=""kc_params"" id=""kc_params"" style=""width:400px;height:80px;"">"&formencode(data(13,0))&"</textarea>"
	Il king.check("")
	Il "</p>"
	'content
	Il king.form_editor("kc_content",kc.lang("label/content")&" <i>{king:content/}</i>",data(14,0),"kc_content|0|"&encode(kc.lang("check/content1")))
	'lastdate
	Il "<p><label>"&kc.lang("label/lastdate")&" <i>{king:lastdate/}</i></label><input maxlength="""" type=""text"" name=""kc_lastdate"" id=""kc_lastdate"" value="""&formencode(data(15,0))&""" class=""in3"" />"
	Il king.check("")
	Il king.form_eval("kc_lastdate",formatdate(now(),0))
	Il "</p>"

	'kpath
	if len(kid)>0 then'更新
		checkpath=";kpath|9|"&encode(kc.lang("check/path2"))&"|select count(kid) from king__movie_page where listid="&listid&" and kpath='$pro$' and kid<>"&kid
	else
		checkpath=";kpath|9|"&encode(kc.lang("check/path2"))&"|select count(kid) from king__movie_page where listid="&listid&" and kpath='$pro$'"
	end if
	Il "<p><label>"&kc.lang("label/path")&" <i>{king:path/}</i></label><input maxlength=""255"" type=""text"" name=""kpath"" id=""kpath"" value="""&formencode(data(8,0))&""" class=""in4"" />"
	if len(kid)=0 then
		maxkid=king.neworder("king__movie_page","kid")
	...
	if king.instre(king.plugin,"passport") then
		...
		set rs=conn.execute("select groupnum,groupname from king__movie_group order by groupnum")
			if not rs.eof and not rs.bof then
				idata=rs.getrows()
				for i=0 to ubound(idata,2)
					kgrade=kgrade&"|"&idata(0,i)&":"&encode(htmlencode(idata(1,i)))
	...
	'moviekeyword
	king.form_input "kkeywords",kc.lang("label/keyword")&" <i>{king:keywords/}</i>",data(6,0),"kkeywords|6|"&encode(kc.lang("check/keyword"))&"|0-100"
	'kdescription
	king.form_area "kdescription",kc.lang("label/description")&" <i>{king:description/}</i>",data(7,0),"kdescription|6|"&encode(kc.lang("check/description"))&"|0-255"
	...
	if king.ischeck and king.ismethod then
		...
		if len(data(7,0))>0 then
			...
			kdescription=king.lefte(king.clshtml(data(14,0)),250)
		end if
		'kkeywords
		kkeywords=king.key(data(0,0)&","&kdescription,data(6,0))
		if cstr(form("snapimg"))="1" then data(14,0)=king.snap(data(14,0))
		'kpath
		...
		if data(8,0)="MD5" then
			if len(kid)>0 then
				...
				data(8,0)=md5(salt(10)&king.neworder("king__movie_page","korder"),0)
			end if
		end if
		'listids
		...
		kc_image=data(12,0)
		if validate(kc_image,5) then
			data(12,0)=king.inst&king_upath&"/image/"&kc.path&"/"&formatdate(date(),2)&"/"&int(timer()*100)&"."&king.extension(kc_image)
			king.createfolder data(12,0)
			king.remote2local kc_image,data(12,0)
		end if
...
'	sql="ktitle,kshow,kcommend,kup,khead,kgrade,kkeywords,kdescription,kpath,listids,kc_actor,kc_director,kc_image,kc_params,kc_content,kc_lastdate"
		...
		if len(kid)>0 then
			...
			conn.execute "update king__movie_page set ktitle='"&safe(data(0,0))&"',kshow="&safe(data(1,0))&",kcommend="&safe(data(2,0))&" ,kup="&safe(data(3,0))&",khead="&safe(data(4,0))&",kgrade="&safe(data(5,0))&",kpath='"&safe(data(8,0))&"',listids='"&safe(data(9,0))&"',listid="&listid&",kdescription='"&safe(kdescription)&"',kkeywords='"&safe(kkeywords)&"'"&insql&" where kid="&kid&";"
		else
			if len(sql_field)>0 then
				fields=split(sql_field,",")
				for i=0 to ubound(fields)
			...
			conn.execute "insert into king__movie_page ("&sql&",kdate,korder,listid) values ('"&safe(data(0,0))&"',"&safe(data(1,0))&","&safe(data(2,0))&","&safe(data(3,0))&","&safe(data(4,0))&","&safe(data(5,0))&",'"&safe(kkeywords)&"','"&safe(kdescription)&"','"&safe(data(8,0))&"','"&safe(data(9,0))&"'"&insql&",'"&tnow&"',"&king.neworder("king__movie_page","korder")&","&listid&")"
			kid=king.newid("king__movie_page","kid")
...
'			king.createrss
'			kc.createmappage listid
		end if
		'neworder：king.neworder("king__movie_page","korder")
		kc.createpage kid
...
sub king_edtlist()
	...
	if king.ismethod or len(listid)=0 then
		...
		set rs=conn.execute("select "&sql&" from king__movie_list where listid="&listid&";")
			if not rs.eof and not rs.bof then
				data=rs.getrows()
			else
				king.error king.lang("error/invalid")
	...
	if len(listid)>0 then'更新
		checkpath="listpath|6|"&encode(kc.lang("check/path"))&"|1-100;listpath|15|"&encode(kc.lang("check/path1"))&";listpath|9|"&encode(kc.lang("check/path2"))&"|select count(listid) from king__movie_list where listpath='$pro$' and listid<>"&listid
	else
		checkpath="listpath|6|"&encode(kc.lang("check/path"))&"|1-100;listpath|15|"&encode(kc.lang("check/path1"))&";listpath|9|"&encode(kc.lang("check/path2"))&"|select count(listid) from king__movie_list where listpath='$pro$'"
	end if
	...
	king.form_input "listkeyword",kc.lang("label/keyword"),data(7,0),"listkeyword|6|"&encode(kc.lang("check/keyword"))&"|0-120"
	king.form_area "listdescription",kc.lang("label/description"),data(8,0),"listdescription|6|"&encode(kc.lang("check/description"))&"|0-250"

	king.form_tmp "listtemplate1",kc.lang("label/listtemplate1"),data(2,0),0
	king.form_tmp "listtemplate2",kc.lang("label/listtemplate2"),data(3,0),kc.path&"[list]"
	king.form_tmp "pagetemplate1",kc.lang("label/pagetemplate1"),data(4,0),0
	king.form_tmp "pagetemplate2",kc.lang("label/pagetemplate2"),data(5,0),kc.path&"[page]"
	...
	if king.ischeck and king.ismethod then
		if len(listid)>0 then
			conn.execute "update king__movie_list set listname='"&safe(data(0,0))&"',listpath='"&safe(data(1,0))&"',listtemplate1='"&safe(data(2,0))&"',listtemplate2='"&safe(data(3,0))&"',pagetemplate1='"&safe(data(4,0))&"',pagetemplate2='"&safe(data(5,0))&"',listid1="&safe(data(6,0))&",listkeyword='"&safe(data(7,0))&"',listdescription='"&safe(data(8,0))&"',listtitle='"&safe(data(9,0))&"',listcontent='"&safe(data(10,0))&"' where listid="&listid&";"
		else
			conn.execute "insert into king__movie_list ("&sql&",listorder) values ('"&safe(data(0,0))&"','"&safe(data(1,0))&"','"&safe(data(2,0))&"','"&safe(data(3,0))&"','"&safe(data(4,0))&"','"&safe(data(5,0))&"',"&safe(data(6,0))&",'"&safe(data(7,0))&"','"&safe(data(8,0))&"','"&safe(data(9,0))&"','"&safe(data(10,0))&"',"&king.neworder("king__movie_list","listorder")&")"
		end if
		kc.createmap
...
sub king_updown()
	...
	if len(kid)>0 then
		king.updown "king__movie_page,kid,korder",kid,"listid="&listid
	else
		king.updown "king__movie_list,listid,listorder",listid,0
	end if
end sub
