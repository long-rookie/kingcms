...
sub king_def()
	...
	if len(query)>0 then
		...
		sql="select top 1000 kid,listid,ktitle,kdescription,kdate,kgrade,kpath from king__{OO}_page where "&query&" order by kid desc;"
		qcount=conn.execute("select count(kid) from king__{OO}_page where kshow=1 and "&query&";")(0)
		...
		set dp=new record
			...
			if dp.length>=0 then
				...
				for i=0 to dp.length
					set rs=conn.execute("select listname,listpath from king__{OO}_list where listid="&dp.data(1,i)&";")
